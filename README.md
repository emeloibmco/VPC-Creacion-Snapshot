...
*Snapshots for VCP* (instantáneas para *VPC*) son una oferta regional que se utiliza para crear una copia puntual de un volumen. Puede tomar una instantánea de un volumen de datos o de arranque que esté adjunto a una instancia de servidor virtual (*VSI*) en ejecución. 

Para crear una instantánea, el volumen original debe estar adjunto a una instancia de servidor virtual en ejecución. La primera instantánea es una copia de seguridad completa del volumen. Las instantáneas posteriores del mismo volumen registran solo los cambios desde la última instantánea. Puede utilizar una instantánea de un volumen de arranque para aprovisionar una instancia. Si la instantánea era de un volumen de datos, puede adjuntarla a una instancia de servidor virtual. Puede crear un nuevo volumen a partir de una instantánea (lo que se denomina restaurar un volumen). Las instantáneas son persistentes; tienen un ciclo de vida independiente del volumen original.

La presente guía muestras los pasos que se deben realizar para generar un *Snapshot* que permita crear una copia puntual del volumen arranque de *Block Storage* de una *VSI*.
